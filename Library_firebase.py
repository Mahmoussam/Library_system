...
class Library():
    ...
    def __init__(self,title):
        """library constructor
            ...
            builds required data and structures in single loop iteration from source FireBase json data  and sets required initial instance members beside UI initialization"""
        self._title=title
        self._sections=[]
        self._profit=0
        self._sections_dict={}
        ...
        #build data from firebase with default conifg
        self.fb=FireBase(self.default_config)
        data=self.fb.raw_load_books()
    ...
    def sell_btn_clicked(self):
        if self._last_chosen_row !=-1:
            book_title=self._current_table_list[self._last_chosen_row][0]
            self.sellaBook(book_title)
            #remove from firebase
            self.fb.delete_book(book_title)
        self.refresh_ui_data()
    def start_all(self):
